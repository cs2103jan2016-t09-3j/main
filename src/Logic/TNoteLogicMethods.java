...
public interface TNoteLogicMethods {
	...
	// can return either a boolean (cause u alrdy have the new directory if u really want
	// else can return the new directory string (might be safer cos i can append the full root directory
	public String changeDirectory(ArrayList<String> parserOutput);
	
	public String deleteDirectory(ArrayList<String> parserOutput);
	
	//Adam return return joelle ur logic command object so she knows what action was performed.
	...
	//for recurring, we gonna make a new object but it extends TaskFile. Joelle u can just work assuming
	//its a TaskFile. The additional method calls is just a way to get the keyword for u
	//ie. every DAY or every TUESDAY etc. u will get an accessor to the word in caps.
